...
// ============================================
// GRÁFICOS DE MONITOREO DE SISTEMA
// ============================================

function inicializarGraficosMonitoreo() {
    // Gráfico de consumo en tiempo real
    const ctx1 = document.getElementById('chartConsumoTiempoReal');
    if (ctx1) {
        const horas = [];
        const consumos = [];
        ...
        for (let i = 23; i >= 0; i--) {
            ...
            consumos.push(Math.random() * 50 + 30);
        }
    ...
    const ctx2 = document.getElementById('chartActividadUsuarios');
    if (ctx2) {
        const usuarios = JSON.parse(localStorage.getItem('energix_usuarios') || '[]');
        const nombres = usuarios.map(u => u.nombre.split(' ')[0]);
        const actividad = usuarios.map(() => Math.floor(Math.random() * 100));
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: function(context) {
                                return context.label + ': ' + context.parsed + ' acciones';
                            }
                        }
                    }
                }
